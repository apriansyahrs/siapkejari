...
@section('content')
                    ...
                    <div class="col-auto ms-auto d-print-none">
                        ...
                        <div class="btn-list">
                            <a href="#" class="btn bg-transparent border border-secondary text-dark"
                                ...
                                Rekap Single
                            </a><a href="#" class="btn bg-transparent border border-secondary text-dark"
                                data-bs-toggle="modal" data-bs-target="#reportMonth">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round"
                                    class="icon icon-tabler icons-tabler-outline icon-tabler-report">
                                        ...
                                        @if ($attendances->count() > 0)
                                            @foreach ($attendances as $index => $item)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a
                                                            href="{{ route('panel.attendance.show', $item->id) }}">{{ $item->employee->name }}</a>
                                                    </td>
                                                    <td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3" id="checkin-time-container" style="display: none;">
                        <label class="form-label">Checkin Time</label>
                        <input type="time" class="form-control" name="checkin_time">
                        <div class="invalid-feedback"></div>
                    </div>
                    <div class="mb-3" id="checkout-time-container" style="display: none;">
    ...
    <div class="modal modal-blur fade" id="report" tabindex="-1" role="dialog" aria-hidden="true"
        data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="period" autocomplete="off" maxlength="7"
                            readonly>
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
    ...
    <div class="modal modal-blur fade" id="reportMonth" tabindex="-1" role="dialog" aria-hidden="true"
        data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.update-btn').forEach(button => {
                button.addEventListener('click', async function() {
                    ...
                    const checkinTimeInput = document.querySelector(
                        `input[name="edit_checkin_time_${id}"]`);
                    const checkoutTimeInput = document.querySelector(
                        `input[name="edit_checkout_time_${id}"]`);
                    const updateButton = document.getElementById(`update-${id}`);
                    const closeButton = this.closest('.modal').querySelector(
                        'button.btn-close');
                    ...
                    const enableEditElements = () => {
                        ...
                        updateButton.innerHTML =
                            `<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 12l5 5l10 -10" /></svg> Simpan`;
                    };
                    ...
                    const showEditErrors = (errors) => {
                        if (errors.checkin_time) {
                            checkinTimeInput.classList.add('is-invalid');
                            checkinTimeInput.nextElementSibling.textContent = errors
                                .checkin_time[0];
                        }
                        if (errors.checkout_time) {
                            checkoutTimeInput.classList.add('is-invalid');
                            checkoutTimeInput.nextElementSibling.textContent = errors
                                .checkout_time[0];
                        }
                    };
                    ...
                    const updateAttendance = async () => {
                        ...
                        const response = await fetch(
                            `${window.location.origin}/panel/presensi/${id}`,
                            options);
                        const result = await response.json();
                        return result;
                    };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const checkoutTimeContainer = document.getElementById('checkout-time-container');

            // Function to show or hide checkin and checkout time fields based on selected status
            const toggleCheckinCheckoutFields = () => {
                if (statusInput.value === 'Hadir') {
                    checkinTimeContainer.style.display = 'block';
                    checkoutTimeContainer.style.display = 'block';
                } else {
                    checkinTimeContainer.style.display = 'none';
                    checkoutTimeContainer.style.display = 'none';
                }
            };

            // Event listener for status change
            statusInput.addEventListener('change', toggleCheckinCheckoutFields);

            // Initial call to set the correct visibility when the page loads
            toggleCheckinCheckoutFields();
        });
    </script>
@endpush
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="time" class="form-control" name="edit_checkin_time_{{ $item->id }}"
                            value="{{ $item->checkin_time !== '00:00:00' ? \Carbon\Carbon::parse($item->checkin_time)->format('H:i') : '' }}">
                        <div class="invalid-feedback"></div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Jam Keluar</label>
                        <input type="time" class="form-control" name="edit_checkout_time_{{ $item->id }}"
                            value="{{ !in_array($item->checkout_time, ['00:00:00', null]) ? \Carbon\Carbon::parse($item->checkout_time)->format('H:i') : '' }}">
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button id="update-{{ $item->id }}" type="button"
                        class="btn btn-primary ms-auto update-btn" data-id="{{ $item->id }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
