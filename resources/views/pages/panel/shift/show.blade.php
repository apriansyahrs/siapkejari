                ...
                <div class="col-lg-5">
                    <div class="row">
                        ...
                        <div class="col-8 col-lg-8 text-end">
                            @if ($shift->is_active)
                            <form action="{{ route('panel.shift.deactivate', $shift->id) }}" method="post" class="d-inline-block">
                                @csrf
                                @method('PUT')
                            ...
                            @else
                            <form action="{{ route('panel.shift.activate', $shift->id) }}" method="post" class="d-inline-block">
                                @method('PUT')
                                @csrf
                            ...
                            <form action="{{ route('panel.shift.destroy', $shift->id) }}" method="post" class="d-inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus shift?')">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row my-3">
                                <div class="col-6">
                                    ...
                                    <input type="time" class="form-control" name="checkin-time" value="{{ \Carbon\Carbon::parse($shift->checkin_time)->format('H:i') }}">
                                    <div class="invalid-feedback"></div>
                                </div>
                                <div class="col-6">
                                    <label for="" class="form-label">Jam Keluar</label>
                                    <input type="time" class="form-control" name="checkout-time" value="{{ \Carbon\Carbon::parse($shift->checkout_time)->format('H:i') }}">
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
...
<script>
    ...
    saveButton.addEventListener('click', async () => {
        ...
        if (response.status === 'success') {
            return window.location.href = `${window.location.origin}/panel/shift`;
        }
    ...
    const save = async () => {
        ...
        const response = await fetch(`${window.location.origin}/panel/shift/${id}`, options);
